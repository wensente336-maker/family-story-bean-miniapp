...
import {
  createHighlightComment, deleteHighlightComment, listHighlightComments,
  updateHighlightComment, type HighlightComment,
} from "../services/highlightApi";

export function PostcardComments({workId,onCountChange}:{workId:string;onCountChange:(delta:number)=>void}){
  const[items,setItems]=useState<HighlightComment[]>([]),[cursor,setCursor]=useState<string|null>(null),[text,setText]=useState(""),[editing,setEditing]=useState<HighlightComment|null>(null),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[error,setError]=useState("");
  const load=async(next?:string)=>{setLoading(true);setError("");try{const page=await listHighlightComments(workId,next);setItems(current=>next?[...current,...page.items]:page.items);setCursor(page.next_cursor)}catch{setError("评论暂时无法读取，请稍后重试。")}finally{setLoading(false)}};
  useEffect(()=>{void load()},[workId]);
  const submit=async(event:FormEvent)=>{event.preventDefault();const body=text.trim();if(!body||busy)return;setBusy(true);setError("");try{if(editing){const saved=await updateHighlightComment(editing.id,body);setItems(value=>value.map(item=>item.id===saved.id?saved:item));setEditing(null)}else{const saved=await createHighlightComment(workId,body);setItems(value=>[saved,...value]);onCountChange(1)}setText("")}catch{setError("评论未能保存，请重试。")}finally{setBusy(false)}};
  const remove=async(comment:HighlightComment)=>{setBusy(true);try{await deleteHighlightComment(comment.id);setItems(value=>value.filter(item=>item.id!==comment.id));onCountChange(-1);if(editing?.id===comment.id){setEditing(null);setText("")}}catch{setError("评论未能删除，请重试。")}finally{setBusy(false)}};
